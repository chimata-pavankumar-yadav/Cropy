...
function CustomHead({ navigation, header }) {
    return (
        <View style={{ flexDirection: 'row', height: 60, borderWidth: 1, borderColor: 'black' }}>

            <TouchableOpacity onPress={() => navigation.goBack()} style={{ justifyContent: 'center', flex: .5, paddingLeft: 10, paddingTop: 5 }}>
                <Ionicons name="md-arrow-round-back" size={28} color="gray" />
            </TouchableOpacity>

            <View style={{ flex: 2, alignItems: 'center', justifyContent: 'center' }}><Text style={{ fontSize: 21, fontWeight: 'bold', color: 'white' }}>{header}</Text></View>
    ...
    return (
        ...
        <View style={{ flex: 1, backgroundColor: 'black' }}>
            <View style={{ marginTop: Constants.statusBarHeight, flex: 1, marginBottom: 30 }}>
                <CustomHead navigation={navigation} header={detail.toUpperCase() + '  PEOPLE'} />
