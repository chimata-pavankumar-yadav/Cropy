...
export default function Tabletab({ personname, onPressItem, datee, mint, hrr }) {
  ...
  useEffect(() => {
    ...
    db.transaction(tx => {
      ...
      tx.executeSql(
        `select * from ` + `` + personname + `` + ` where id IS NOT NULL ;`,
        [],
  ...
  const [temptimehour, setTemptimehour] = useState(null);
  const [temptimeminute, setTemptimeminute] = useState(null);
  const [tempgreentimehour, setTempgreentimehour] = useState(null);
  const [tempgreentimeminute, setTempgreentimeminute] = useState(null);

  const sidedelete = (date) => {

    db.transaction(
      tx => {
        tx.executeSql(`select hour from ` + `` + personname + `` + ` where choosedate = ?`,
          [date],
          (_, { rows: { _array } }) => setTemptimehour(parseInt(_array[0].hour.slice(0, 2).split('.'))),
        );
        tx.executeSql(`select minutes from ` + `` + personname + `` + ` where choosedate = ?`,
          [date],
          (_, { rows: { _array } }) => setTemptimeminute(parseInt(_array[0].minutes.slice(0, 2).split('.'))),
        );
        tx.executeSql(`delete from ` + `` + personname + date[1] + `` + ` ;`, []);
        tx.executeSql(
          `select ghour from ` + `` + personname + `greentable` + `` + ` where id = 1;`,
          [],
          (_, { rows: { _array } }) => setTempgreentimehour(parseInt(_array[0].ghour)),
        );
        tx.executeSql(
          `select gminutes from  ` + `` + personname + `greentable` + `` + ` where id = 1;`,
          [],
          (_, { rows: { _array } }) => setTempgreentimeminute(parseInt(_array[0].gminutes)),
        );
        if (tempgreentimeminute === null && tempgreentimehour === null && temptimehour === null && temptimeminute === null) {
          return false;
        }
      },
  ...
  if (tempgreentimeminute !== null) {
    if (tempgreentimeminute < temptimeminute) {
      var ttm = temptimeminute - tempgreentimeminute
    } else {
      var ttm = tempgreentimeminute - temptimeminute
    }
    var tth = tempgreentimehour - temptimehour
  ...
  if (sidebox !== null && count === 0) {
    try {
      setClor(sidebox[0].id)
      setCount(1)
      setPlus(false)
      db.transaction(
        tx => {
          tx.executeSql(
            `select * from ` + `` + personname + sidebox[0].choosedate[1] + `` + ` ;`,
            [],
            (_, { rows: { _array } }) => setAlltimearray(_array),
          );
        });
                    ...
                    onPress={() =>
                      Alert.alert(
                        ...
                        [
                          ...
                          {
                            text: "DELETE", onPress: () =>
                              [sidedelete(item.choosedate), onPressItem && onPressItem(item.id)]
