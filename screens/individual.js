...
function Customindividual({ navigation, header }) {
    return (
        <View style={{ flexDirection: 'row', height: 60, }}>
            ...
            <TouchableOpacity onPress={() => navigation.goBack()} style={{ justifyContent: 'center', flex: 1, paddingLeft: 10, paddingTop: 5 }}>
                <Ionicons name="md-arrow-round-back" size={28} color="gray" />
            </TouchableOpacity>

            <View style={{ flex: 7, justifyContent: 'center', alignItems: 'center', marginRight: 17, marginTop: 5 }}><Text style={{ fontSize: 21, fontWeight: 'bold', color: 'white' }}>{header}</Text></View>
...
export default function IndividualsInfo({ route, navigation }) {
    ...
    useEffect(() => {
        db.transaction(tx => {
            tx.executeSql(
                "create table if not exists " + "" + details.name.replace(/ +/g, '') + "" + " (id integer primary key not null, advance text, hour text,minutes text,choosedate text,wheeler text);"
            );
            tx.executeSql(
                `select * from ` + `` + details.name.replace(/ +/g, '') + `` + ` ;`,
                [],
    ...
    const nData = (data) => {
        ...
        db.transaction(
            tx => {
                if (data) {
                    tx.executeSql("insert into " + "" + details.name.replace(/ +/g, '') + "" + " (advance,hour,minutes,choosedate,wheeler) values (?,?,?,?,?)", [data.advance, 0, 0, choosedate, checked]);
                    tx.executeSql(
                        `select * from ` + `` + details.name.replace(/ +/g, '') + `` + ` ;`,
                        [],
                        (_, { rows: { _array } }) => setDetai(_array),
    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'black' }}>
            <View style={{ marginTop: Constants.statusBarHeight, flex: 1, marginBottom: 100 }}>
                <Customindividual navigation={navigation} header={details.name + ' Info'} />
                ...
                <View style={{ marginTop: 15, }}>
                    ...
                    <Tabletab tabletabname={detai} datee={details.choosedate} hrr={details.hour} mint={details.minutes} personname={details.name.replace(/ +/g, '')} key={`forceupdate-todo-${forceUpdateId}`} onPressItem={id =>
                        db.transaction(
                            tx => {
                                {
                                    (id > 0) ?
                                        (tx.executeSql(`delete from ` + `` + details.name.replace(/ +/g, '') + `` + ` where id = ?;`, [id]),
                                            tx.executeSql(
                                                `select * from ` + `` + details.name.replace(/ +/g, '') + `` + ` ;`,
                                                [],
                                                (_, { rows: { _array } }) => setDetai(_array),
