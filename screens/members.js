...
function CustomHead({ navigation, header }) {
  return (
    <View style={{ flexDirection: 'row', height: 60, borderWidth: 1, }}>
      ...
      <View style={{ flex: 1.5, alignItems: 'center', justifyContent: 'center' }}><Text style={{ fontSize: 21, fontWeight: 'bold', color: 'white' }}>{header}</Text></View>
  ...
  return (
    ...
    <View style={{ flex: 1, backgroundColor: 'black' }}>
      <View style={{ marginTop: Constants.statusBarHeight, flex: 1, marginBottom: 30, }}>
        <CustomHead navigation={navigation} header={'PLACES'} />
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity style={styles.text} onPress={() => navigation.navigate('Indiviplace', item.place)} >
                ...
                <Text style={{ color: 'white', fontSize: 17, fontWeight: 'bold' }}>{item.place}</Text>
